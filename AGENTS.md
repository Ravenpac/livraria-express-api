...
| Run tests            | `npm test` (Jest, needs `--experimental-vm-modules` for ESM) |
...
- **ESLint** configured (flat config in `eslint.config.js`) — `no-console` as `warn`, `no-unused-vars` as `error` (ignoring `_`-prefixed args); Jest globals in test files via `globals.jest`
- **Jest 30** with ESM — `jest.config.js` sets `transform: {}`; `npm test` uses `--experimental-vm-modules` flag; create `.test.js` files alongside source
